...
W = tf.Variable(tf.random_uniform([observations,actions],0,0.01))
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        logging.debug("Episode number {0} completed with reward {1}".format(i, rAll))
        print("Episode number {0} completed with reward {1}".format(i, rAll))

        if i%100 == 0:
            logging.debug("Average reward after {0} episodes = : {1} ".format(i, str(sum(rList)/(i+1))))
            logging.debug("time elapsed in seconds = ", time.clock())
            logging.debug("The weights after {0} episodes are = : {1} ".format(i, W1))
